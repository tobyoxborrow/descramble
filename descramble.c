...
void loadWordList(int length,char words[][MAX_WL][MAX_WORDN],char *gridAsString)
{
	char filename[9];
...
int main(int argc, char *argv[])
{
	...
	// populate grid from command line
	if (argc >= 5) {
		int k;
		for (k = 0; k < 4; k++) {
			if (strlen(argv[k+1]) != 4) {
				usage(argv[0]);
				return 0;
			}
			strncpy(grid[k], argv[k+1], 4);
		}
		sprintf(gridAsString,"%s%s%s%s",argv[1],argv[2],argv[3],argv[4]);
	} else {
		usage(argv[0]);
		return 0;
	}
